...
def plot_sample_size_exp(ns_data, coveragedf, path):

  rs_oracle_up = ns_data['Rs_up_oracle'].tolist()[-1]
  rs_oracle_down = ns_data['Rs_down_oracle'].tolist()[-1]
  rd_oracle_up = ns_data['Rd_up_oracle'].tolist()[-1]
  rd_oracle_down = ns_data['Rd_down_oracle'].tolist()[-1]
  ...
  # Coverage plot
  samples = coveragedf[(coveragedf['est_method'] == 'plugin') & (coveragedf['metric'] == 'm_a=0')]
  rs_coverage = samples.groupby('N').mean()['Rs_coverage'].tolist()[:-1]
  rd_coverage = samples.groupby('N').mean()['Rd_coverage'].tolist()[:-1]
  N = samples.groupby('N').mean().reset_index()['N'].tolist()[:-1]

  ax1.plot(N, rs_coverage, marker='o', color='red', linewidth=3, label='Baseline interval')
  ax1.plot(N, rd_coverage, marker='o', color='blue', linewidth=3, label='Delta interval (ours)')
  ax1.set_ylabel('Coverage (%)', fontsize=14)
  ax1.legend()


  # Interval estimation plot
  ax2.axhline(rs_oracle_up, color='red', linestyle='--', label='Baseline interval')
  ax2.axhline(rs_oracle_down, color='red', linestyle='--')
  ax2.axhline(rd_oracle_up, color='blue', linestyle='--', label='Delta interval (ours)')
  ax2.axhline(rd_oracle_down, color='blue', linestyle='--')
  ax2.axhline(oracle, color='k', linewidth=2,label='Oracle')

  # Standard interval
  ax2.scatter(ns_data['N'], ns_data['Rs_up_pl_mean'], color='red')
  for ix,n in enumerate(ns_data['N']):
      ax2.errorbar(n, ns_data['Rs_up_pl_mean'].tolist()[ix], yerr=ns_data['Rs_up_pl_ci'].tolist()[ix],capsize=4, linewidth=3, elinewidth=3, color='red')

  ax2.scatter(ns_data['N'], ns_data['Rs_down_pl_mean'], color='red')
  for ix,n in enumerate(ns_data['N']):
      ax2.errorbar(n, ns_data['Rs_down_pl_mean'].tolist()[ix], yerr=ns_data['Rs_down_pl_ci'].tolist()[ix],capsize=4, linewidth=3, elinewidth=3, color='red')
  ...
  ax2.legend(loc='lower center', bbox_to_anchor=(0.5, .9), ncol=3)

  plt.savefig(path, dpi=500, bbox_inches='tight')


def plot_fig3_intervals(df, metrics, ix_y):
    ...
    for i, metric in enumerate(metrics):
        ...
        # Filter the DataFrame for the current metric
        mdf = df[df['metric'] == metric].iloc[0]
